...
variable "vm_name" {
  description = "Name of the Customer and Asset Service VM."
  type        = string
  default     = "vm-assms-customer-staging"
}
variable "vm_size" {
...
variable "source_image_sku" {
  description = "Canonical Ubuntu Arm64 image SKU for the staging service VM."
  type        = string
...
variable "nsg_name" {
  ...
  type        = string
  default     = "nsg-assms-customer-staging"
}
variable "nic_name" {
  description = "Name of the service NIC."
  type        = string
  default     = "nic-assms-customer-staging"
}
variable "public_ip_name" {
  description = "Name of the optional service public IP."
  type        = string
  default     = "pip-assms-customer-staging"
}
variable "public_ip_domain_name_label" {
  description = "Optional Azure Public IP DNS label for the staging HTTPS endpoint."
  type        = string
  default     = null
  nullable    = true
}
...
variable "http_allowed_source_cidrs" {
  description = "CIDRs allowed to connect over HTTP/HTTPS when enabled; 0.0.0.0/0 is permitted only for the public HTTPS endpoint and ACME HTTP-01 validation."
  type        = list(string)
  default     = []
}
variable "tags" {
  ...
  default = {
    Project     = "ASSMS"
    Environment = "staging"
    Service     = "Customer and Asset Service"
    ManagedBy   = "Terraform"
  }
}
